using System;

public class Departure
{
	public Departure()
    {//view of the airline flight information about arrivals and departures
     // date and time, flight number, city/ port of
     //arrival(departure),      
     // terminal, flight status(check-in, gate closed,
     //arrived, departed at, unknown, canceled, expected at, delayed, in flight,gate)  

        int DateandTime = new DateandTime();
        int FlightNumber = new FlightNumber();
        string City = new City();
        char Terminal = new Terminal();
        int Gate = new Gate();
        string FlightStatus = new FlightStatus();

        ЯНеправильноеИмяКласса к = new СеренькийКозлик();

    }
}
